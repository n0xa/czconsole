...
# Patch operator username in units that run as the operator (files) when the
# operator is not 'kali' (e.g. FILES_USER=pi on stock Raspberry Pi OS).
if [ "${FILES_USER:-kali}" != "kali" ]; then
  for svc in czconsole-files.service; do
